...
#' 
#' @source \href{https://github.com/Bioconductor/GenomeInfoDbData/blob/master/data/specData.rda}{
...
get_orgdb_genomeinfodbdata <- function(verbose=TRUE){ 
  ...
  gh <- paste0("gi","th","ub")
  URL <- paste0("https://",gh,".com/Bioconductor/GenomeInfoDbData/raw/master/data/specData.rda")
  specData <- load_data(filename = URL, 
                        verbose = verbose)
  ...
  specData[,scientific_name:=format_species_name(display_name, replace_char = "_")]
  specData[,id:=format_species_name(scientific_name, gs_s = TRUE, split_char = "_", lowercase=TRUE)]
  specData[,version:=NA] 
  data.table::setnames(specData,"tax_id", "taxonomy_id")
  keep_cols <- c("display_name","id","scientific_name","taxonomy_id","version")
