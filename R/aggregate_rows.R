...
aggregate_rows <- function(X, 
  ...
  if(tolower(method[1])=="stats"){
    X_agg <- aggregate_rows_stats(X=X,
                                  groupings=groupings,
                                  FUN=FUN,
                                  dropNA=dropNA)
    
  } else if(tolower(method[1])=="monocle3"){
    X_agg <- aggregate_rows_monocle3(x=X,
                                     groupings=groupings, 
                                     fun=FUN, 
                                     na.action=if(dropNA){stats::na.omit
                                       } else {stats::na.pass}
                                     )
    
  } else if(tolower(method[1])=="delayedarray"){
    X_agg <- aggregate_rows_delayedarray(X = X, 
                                         labels = groupings)
    
  } else {
    method_opts <- c("stats","monocle3","delayedarray")
    stop_msg <- c("method must be one of:\n",paste("  -",method_opts,collapse = "\n"))
    stop(stop_msg)
  }
  ...
  if(as_sparse){X_agg <- methods::as(X_agg,"sparseMatrix")}
  if(as_DelayedArray){X_agg <- DelayedArray::DelayedArray(X_agg)}
  return(X_agg)
}