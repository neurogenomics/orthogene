...
map_orthologs_babelgene <- function(genes,
    ...
    check_species_babelgene(source_id = source_id, 
                            target_id = target_id)
    #### Return input genes ####
    if(all(is_human(source_id), is_human(target_id))){
        messager("input_species==output_species.",
                 "Returning input genes.",v=verbose)
        gene_map <- data.frame(input_gene=genes, 
                               ortholog_gene=genes)
        return(gene_map)
    }
    #### Convert ####
    ...
    if((!is_human(source_id)) & (is_human(target_id))){ 
        gene_map <- gene_map %>% dplyr::select(input_gene=symbol,
    ...
    #### Human ==> non-human ####
    if((is_human(source_id)) & (!is_human(target_id))){ 
        gene_map <- gene_map %>% dplyr::select(input_gene=human_symbol,
