...
plot_benchmark_bar <- function(bench_res,
  ...
  time_plot <- ggplot(bench_res, 
    ...
    geom_point(aes(color=method), position=position_dodge(width=.9),
  ...
  genes_plot <-ggplot(bench_res, 
                      aes(x=species, y=genes, 
                          fill=method, shape=test)) +
    scale_fill_manual(values = fill_colors, drop=FALSE) + 
    scale_color_manual(values = fill_colors, drop=FALSE) + 
    geom_bar(stat='identity', position = "dodge", alpha=.7, 
  ...
  all_genes_plots <- (time_plot / genes_plot) + patchwork::plot_annotation(tag_levels = letters)
