...
#' @param drop_nonorths Drop genes that don't have an ortholog in the \code{output_species}. 
...
#' @param genes_as_rownames Whether to return the data.frame with the row names
#'  set to the human orthologs.
#' @param return_dict Return a dictionary (named list) of gene mappings, 
#' where \code{names} are the original gene names and items are the new gene names. 
#' If \code{return_dict=FALSE}, instead returns mappings as a data.frame.
...
convert_orthologs <- function(gene_df,
  ...
  if(genes_as_rownames){
    if(!drop_nonorths){
      messager("WARNING: drop_nonorths must =TRUE in order to set genes_as_rownames=TRUE.")
    }else {
      messager("+ Setting Gene col as rownames...",v=verbose)
      if(!one_to_one_only) stop("Genes must be unique to be row.names. Try again with `one_to_one_only=TRUE`")
