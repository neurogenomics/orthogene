test_that("convert_orthologs works", {
  data("exp_mouse") 
  gene_df1 <- convert_orthologs(gene_df = data.frame(as.matrix(exp_mouse)), 
                                input_species="mouse",
                                gene_col="rownames",
                                drop_nonorths = TRUE)
  ...
  n_human_genes <- sum(gene_df1$ortholog_gene %in% human_genes2)
  n_mouse_genes <- sum(rownames(gene_df1) %in% mouse_genes2)
