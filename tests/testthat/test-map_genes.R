test_that("map_genes works", {
  ...
  mapped_human <- map_genes(genes=genes,
                            species="human",
                            drop_na=TRUE)
  mapped_mouse <- map_genes(genes=genes,
                            species="mouse",
                            drop_na=TRUE)
  mapped_zebrafish <- map_genes(genes=genes,
                                species="zebrafish",
                                drop_na=TRUE)
  mapped_fly <- map_genes(genes=genes,
                          species="fly", 
                          drop_na=TRUE)
  data("exp_mouse")
  mapped_fly2 <- map_genes(genes=rownames(exp_mouse)[seq(1,1000)],
                          species="fly", 
                          drop_na=TRUE)
