...
A key feature of `convert_orthologs` is that it handles the issue of many-to-many gene mappings across species using different strategies via the `non121_strategy=` argument:  

1. `"drop_both_species"` : All genes that do not have 1:1 orthologs between the species will be dropped, as the function of these genes tends to be less conserved and less interpretable (*DEFAULT*). 
2. `"drop_input_species"` : Only drop genes with multiple orthologs in the `input_species`.  
3. `"drop_output_species"` : Only drop genes with multiple orthologs in the `output_species`. 
